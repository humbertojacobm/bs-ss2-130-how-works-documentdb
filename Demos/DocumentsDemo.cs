...
namespace DocDb.DotNetSdk.Demos
{
	public static class DocumentsDemo
	{
		...
		public async static Task Run()
		{
			...
			{
				Database database = client.CreateDatabaseQuery("SELECT * FROM c WHERE c.id = 'hmori_database'").AsEnumerable().First();
				_collection = client.CreateDocumentCollectionQuery(database.CollectionsLink, "SELECT * FROM c WHERE c.id = 'hmori_collection'").AsEnumerable().First();
